...
function extractFromBlockquote(content) {
  ...
  // 只扫描前 30 行的 blockquote 元数据（标题可能在 blockquote 之前）
  ...
  for (const pattern of sectionPatterns) {
    ...
    if (match) {
      const text = match[0]
        ...
        .replace(/<!--[\s\S]*?-->/g, '')   // 去 HTML 注释（内部工作标注）
...
function extractTags(body, frontmatter, blockquoteTags) {
  ...
  // 2. Blockquote 中的标签（Format B: > **标签**：xxx、xxx）
  if (blockquoteTags) {
    const tags = blockquoteTags.split(/[,，、\s]+/).filter(t => t.length > 0);
...
function parse(content, filePath, pipeline, stage) {
  ...
  if (!date && fm.created) date = normalizeDate(String(fm.created));
  ...
  // 8. 标签
  const tags = extractTags(body, fm, blockquoteMeta.tags);
