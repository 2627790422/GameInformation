...
> ⚠️ 此文档为项目初期设计方案，大部分内容已过时。
> 当前架构和流水线配置见 `../CLAUDE.md`。
...
| 流水线 | 约数量 | 数据特征 |
|--------|--------|----------|
| 微信资讯 | ~60篇 | 微信公众号文章采集+AI分析，Format B（blockquote元数据） |
| AI资讯 | ~50篇 | AI行业资讯 |
| 设计管线 | ~6篇 | 管线产出文件，YAML frontmatter（含 `.harness` 目录跳过） |
| 游戏跟踪 | ~5篇 | 游戏追踪分析 |
| 访谈跟踪 | ~4篇 | 访谈内容已分析 |

所有数据从 ObsidianNote 仓库通过 GitHub API 下载，预构建为 JSON 部署到 Vercel。
